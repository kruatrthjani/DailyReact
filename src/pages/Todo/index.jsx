...
export default function TodoItem() {
  ...
  useEffect(() => {
    const timers = tasks
      ...
      .map((task) => setInterval(() => incrementTimer(task.id), 1000));
  ...
  const addTask = async () => {
    ...
    if (editIndex) {
      const alltask = tasks.map((task) =>
        task.id == editIndex ? { ...task, name: name } : task
      );
      console.log(alltask);
      setTasks(alltask);
      taskcontent();
  ...
  const updateTimer = async (id, type) => {
    const updatedTasks = tasks.map((task) => {
      if (task.id === id) {
        // Check if the task matches the given id
        if (type === "start") {
      ...
      {localTasks.map((task) => (
        ...
        >
          ...
          >
            ...
            {/*button onClick={EditTask}><i className="fa-regular fa-edit"></i></button>*/}
          </p>
          ...
          <Button onClick={() => EditName(task.id)}>Edit</Button>
        </div>
      ))}
